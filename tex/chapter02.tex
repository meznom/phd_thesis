\chapter{\label{ch:approximations}Approximations}
...
\emph{grand canonical} \cgls{QCA} Hamiltonian \eqref{eq:H_QCA}, only devices of
...
gain a better understanding of the \cgls{QCA} approach. As a first step, we
...
simplification. However, it is in line with the \cgls{QCA} idea: the approach
...
implementing \cgls{QCA} on it. For experimental systems like the atomic silicon
...
\cgls{QCA} cell layout considered within the fixed-charge approximation cannot
...
\begin{figure}
  ...
  {
  \label{fig:bond}
  The six bonds of a \cgls{QCA} cell. The basis of a \cgls{QCA} cell in the
...
\end{figure}
%
At its heart, \cgls{QCA} is a semi-classical idea. It relies dominantly on
...
the operation of \cgls{QCA} does not rely on the spin, singlets and triplets
...
the splitting in more detail in due course. For the bond model the \cgls{QCA}
...
\newacronym{SSE}{SSE}{stochastic series expansion}
...
\begin{figure}
  ...
  corresponds to spin down and logic 1 to spin up. (b) \cgls{QCA} cells $k$ and
  $l$. 
  }
\end{figure}
%
A linear array of \cgls{QCA} cells where each cell has a state of logic 0 or 1
...
\cgls{QCA} system to a transverse-field Ising model with long-ranged
...
\cgls{SSE} quantum Monte Carlo schemes \cite{Sandvik2003}. These methods do not
scale exponentially%
%
\footnote{
\cgls{SSE} quantum Monte Carlo methods roughly scale as $N \ln N$ where $N$ is
the system size.
}
%
and consequently allow access to much larger systems. Last, but not least, such
a mapping connects the \cgls{QCA} approach to the established and well studied
Ising model. The prospect hinges on the assumption that the two-states-per-cell
basis actually is a good approximation for \cgls{QCA} systems. And while
bistable two-state cells are certainly the picture we have in mind when we talk
about \cgls{QCA}, it is not \emph{a priori} clear whether this is a correct
...
satisfyingly derived, in the literature to study the dynamics of \cgls{QCA}
systems.

We use the bond Hamiltonian \eqref{eq:H_bond} as the starting point. We had
...
illustrated in Fig.~\ref{fig:ising}(a). We will first look at how the \cgls{QCA}
...
A single \cgls{QCA} cell is described by the time-independent Schr\"odinger
...
effective two-state model for the \cgls{QCA} Hamiltonian. With only two states
...
insights into the characteristics of \cgls{QCA} devices. Therefore, we now
...
introduced term $J^{\prime}_{kl}$ vanishes for $q=\frac{1}{2}$. In that special
...
\cgls{QCA} interaction. With the above equations we can also already look at
...
\cgls{QCA}. For non-zero $J^{\prime}_{kl}$ we would, for example, expect
...
we want. For \cgls{QCA} operation we therefore require charge neutral cells and
a genuine, unmodified Ising model.

To obtain more tangible expressions for $J_{kl}$ and $J^{\prime}_{kl}$ we
...
undesirable for functioning \cgls{QCA} devices. Of course, we find our general
...
In summary, we have successfully mapped the \cgls{QCA} bond Hamiltonian
...
for the \cgls{QCA} Hamiltonian: the fixed-charge model, the bond model, and the
...
\begin{figure}
  ...
  approximative fixed-charge two-cell \cgls{QCA} system. For small energies the
  ...
  (a) Low-energy density of states of a one-cell \cgls{QCA} system for both the
...
detrimental to \cgls{QCA}; a cell occupied by only one or by three electrons,
however, renders \cgls{QCA} non-functional. We often use the fixed-charge model
as the starting point and assume, without further investigation, that a
practical \cgls{QCA} implementation can be tuned to be in the right charge
...
of a single \cgls{QCA} cell for both the fixed-charge and the bond model. The
...
\cgls{QCA} approach itself requires, and we will discuss this in detail in the
next chapter.
...
\begin{figure}
  ...
  charge-neutral \cgls{QCA} system. (a) Low-energy density of states. The Ising model
...
\cgls{QCA} approach in general. Consequently, we will concentrate on
...
\begin{figure}
  ...
  charge-neutral \cgls{QCA} system at $V_1 = 100$ and $V_1 = 200$. (a)(c)
...
also remain constant along the wire. Still, for this particular \cgls{QCA}
...
In this chapter we introduced and established three approximations for
\cgls{QCA} systems: the fixed-charge, the bond, and the Ising model. We usually
...
chosen set of parameters, has to be checked for each potential \cgls{QCA}
implementation on a case by case basis. If the fixed-charge model is not
applicable, then there is also generally no hope of implementing \cgls{QCA} on
...
a very good description of \cgls{QCA} systems at high temperatures. It starts
...
While it is conceptually rewarding to map \cgls{QCA} to an Ising system, we have
...
\cgls{QCA} approach works best, as we will find out in the next chapter. Both
...
our \cgls{QCA} characterization work at finite temperature. With system sizes of
