...
To investigate the astounding quantitative difference between the two $d/a =
...
We have found that the model $P_k \sim \chi^k$ is the right physical picture for
...
% This section's discussion is not very logical: In the open paragraph I say
% that for less than ideal cell-cell response the polarization should be zero
% for long wires --- it is therefore not surprising that V_1crit becomes
% infinite in that limit.
%
% In that light the new insight from the wire polarization plots is that the
% cell-cell polarization is actually the same everywhere inside the wire ---
% which, however, is also not that surprising, from a symmetry point of view.
