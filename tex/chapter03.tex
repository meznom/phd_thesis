...
We choose a simple \cgls{QCA} system, the three-cell wire that we had already
introduced in Fig.~\ref{fig:short_wire} in the second chapter, to investigate
general time-independent characteristics of \cgls{QCA} circuits. Specifically,
...
\cgls{QCA} scheme. However, as our graph shows, this is only strictly true for
...
artefact of the \cgls{ICHA}, which we had introduced in more detail in Chapter
\ref{ch:QCA_intro}. \cgls{ICHA} treats each cell individually in the static
charge mean field of the other cells in the system. In other words, in the
\cgls{ICHA} scheme, for each cell the rest of the system is approximated by an
effective driver cell.

We now fix the driver polarization at $P_\text{D} = 1$ and look at how the
...
interest if our aim is to build functional \cgls{QCA} devices.
...
% optimization data is from notebook.000003
% 3 cells, V_1 = 40, q = 0
% uniform wire: P_out = 0.06, b/a = 1.92
% non-uniform wire: P_out = 0.15, b/a = [1.21, 1.55, 2.25]
...
transport in a wire, which would have to be considered when designing \cgls{QCA}
circuitry. More generally, we should be able to optimize the functionality of
any given non-charge-neutral \cgls{QCA} layout by allowing for slightly
...
seen that, in general, we can only map \cgls{QCA} to a modified Ising model with
...
\begin{figure}
  ...
  Ising $J^{\prime}$. In the non-charge-neutral case \cgls{QCA} does not work at
...
system will never work as a \cgls{QCA} circuit unless all we want to do is build linear
chains of cells. Even in this case the system becomes very fragile with respect
to angular displacement. Thus, for \cgls{QCA} charge-neutral cells, $q=\frac{1}{2}$,
...
\cgls{QCA}. Therefore we will from now on restrict ourselves to $q=\frac{1}{2}$
...
for functional \cgls{QCA} circuits.

We use a cluster mean field approach beyond the single-site \cgls{ICHA} approximation
...
as for the \cgls{ICHA} scheme, we solve a cluster mean field Hamiltonian
...
regime is therefore inhibitive for \cgls{QCA} devices. Above the critical $V_1$
...
to Fig.~\ref{fig:critical_V1}(a), obtained from single-cell \cgls{ICHA}
...
valid scale for finite size \cgls{QCA} systems, presumably of a size comparable
to the number of active cells in the mean field scheme, and certainly $V_1 <
V_{1\textrm{crit}}$ is a range where \cgls{QCA} is non-functional. More
...
will always decreases monotonically---and that \cgls{QCA} systems are limited in
...
large \cgls{QCA} systems, so having a second size bound is not a show stopper.
...
small \cgls{QCA} systems, we calculate the cell polarizations in two- to
...
necessary for functional \cgls{QCA} devices, even for relatively small systems.
...
faster to zero for longer wires. Therefore, for large, functional \cgls{QCA}
...
threshold. More generally, this gives an approximate upper size for \cgls{QCA}
...
picture for \cgls{QCA} wires, and by cranking up the system's parameters we will
...
constantly and fully polarized \cgls{QCA} systems employed in the literature is
...
We have shown explicitly that a set of parameters exists where \cgls{QCA} works
...
twelve cells. For practical \cgls{QCA} implementations, however, it is important
...
20$ really is the lower limit for operational \cgls{QCA} devices, at all
...
$V_1/t$ is a requirement for \cgls{QCA}. This Coulomb energy is set directly by
...
the required large $V_1/t$ ratios simply by making the \cgls{QCA} cells large
...
% TODO: state values of b
%
% t ~ e^b a
%   with b = -0.1982 +- 0.0033719
% V_1 ~ b a^-1
%   with b = 1.243 +- 0.0188579
%
% TODO: put in concrete eV numbers of U, V_1, and t
...
the large parameters required for \cgls{QCA} operation. On the silicon (100)
...
In the last section we identified a set of parameters for which the \cgls{QCA}
...
complex \cgls{QCA} structures than the simple horizontal wire, and we pick the
majority gate---the single most important \cgls{QCA} logic device---as an
...
We gain further insight into the characteristics of the \cgls{QCA} majority gate
...
Because \cgls{QCA} has no in-built directionality, different inputs are not
